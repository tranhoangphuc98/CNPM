...
begin
	declare @id char(12);
	declare @KT date;
	set @KT = ( select ngaySinh from inserted);
	...
	begin
		...
		begin
			...
			begin
				if((year(getdate())- year(@KT) > 15) and (year(getdate())- year(@KT) < 30))
					begin
						exists (select * from inserted)
					end
				
				else if(year(getdate())- year(@KT) = 15)
					begin
						if (MONTH(getdate)- month(@KT) > 0)
							begin
								exists (select * from inserted);
							end
						else if (MONTH(getdate)- month(@KT) = 0)
							begin
								if(day(getdate) - day(@KT) == 0)
									begin
										exists (select * from inserted);
									end
								else 
				...
				else if ((year(getdate())- year(@KT) = 30)
					begin
						if (MONTH(getdate)- month(@KT) < 0)
							begin
								exists (select * from inserted);
